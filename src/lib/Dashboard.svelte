<script lang="ts">
  ...
  import { api, getInforme, type InformeData } from './api';
  ...
  function getColorClass(valoracion: string): string {
    ...
    if (v.includes('SOBRESALIENTE')) return 'text-emerald-400';
    if (v.includes('EXCELENTE')) return 'text-green-400';
    if (v.includes('ACEPTABLE')) return 'text-yellow-400';
    if (v.includes('INSUFICIENTE')) return 'text-red-400';
...
<div class="relative z-10 min-h-screen">
  <!-- Navbar -->
  <nav class="glass-nav border-b border-white/5">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        <div class="flex items-center gap-3">
          ...
          <span class="text-white font-bold text-lg tracking-tight">EduAdmin</span>
        </div>

        <div class="flex items-center gap-4">
          <div class="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/5">
            <div class="w-2 h-2 rounded-full bg-green-400 animate-pulse"></div>
            <span class="text-white/60 text-xs">En linea</span>
          </div>
          <button
            onclick={handleLogout}
            class="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white/70 hover:text-white hover:bg-white/10 transition-all duration-200 text-sm"
          >
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <!-- Welcome Card -->
    <div class="glass-card rounded-3xl p-8 md:p-12 border border-white/10 mb-8">
      <div class="flex flex-col md:flex-row items-start md:items-center gap-6">
        ...
        <div class="flex-1">
          <h1 class="text-3xl md:text-4xl font-bold text-white mb-2">
            Bienvenido, <span class="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-400">{$user?.username || 'Usuario'}</span>
          </h1>
          <div class="flex flex-wrap items-center gap-3 mt-3">
            ...
            <span class="text-white/40 text-sm">{$user?.email || ''}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="glass-card rounded-3xl p-8 border border-white/10">
      <div class="flex items-center gap-4 mb-8">
        ...
        <div>
          <h2 class="text-2xl font-bold text-white">Generar Certificados</h2>
          <p class="text-white/40 text-sm">Selecciona el año académico</p>
        </div>
      </div>
        ...
        {#each years as year}
          <button
            ...
            class="group relative p-5 rounded-2xl border-2 transition-all duration-300 {selectedYear === year
              ...
              : 'border-white/5 bg-white/5 hover:border-purple-500/30 hover:bg-white/10'}"
          >
            ...
            {/if}
            <p class="text-2xl font-bold {selectedYear === year ? 'text-purple-400' : 'text-white group-hover:text-purple-400'} transition-colors">{year}</p>
          </button>
        {/each}
      </div>
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between mb-3">
          <span class="text-white/60 text-sm font-medium">Estudiante seleccionado</span>
          {#if selectedRecord}
            <button onclick={clearSelection} class="text-purple-400 text-xs hover:text-purple-300 transition-colors">
              Cambiar
            </button>
          {/if}
        </div>
        {#if selectedRecord}
          <div class="p-4 rounded-2xl border-2 border-purple-500 bg-purple-500/10">
            <div class="flex items-center gap-4">
              ...
              <div>
                <p class="text-white font-bold">{selectedRecord.nombres2}</p>
                <p class="text-white/40 text-sm">IND: {selectedRecord.ind}</p>
              </div>
            </div>
          </div>
        {:else}
          <button
            onclick={openModal}
            class="w-full p-4 rounded-2xl border-2 border-dashed border-white/10 hover:border-purple-500/30 transition-all duration-300 text-left"
          >
            <div class="flex items-center gap-4">
              ...
              <div>
                <p class="text-white font-medium">Buscar estudiante</p>
                <p class="text-white/30 text-sm">Buscar por nombres2 o IND</p>
              </div>
            </div>
          </button>
        {/if}
    ...
    <!-- Informe Table -->
    {#if isLoadingInforme}
      <div class="glass-card rounded-3xl p-8 border border-white/10 mt-8">
        <div class="flex items-center justify-center py-12">
          <div class="w-8 h-8 border-2 border-purple-500 border-t-transparent rounded-full animate-spin"></div>
        </div>
      </div>
    {:else if informeError}
      <div class="glass-card rounded-3xl p-8 border border-red-500/20 mt-8">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-white font-medium">No se encontró informe</p>
            <p class="text-white/40 text-sm">{informeError}</p>
          </div>
        </div>
      </div>
    {:else if informeData}
      <div class="glass-card rounded-3xl p-8 border border-white/10 mt-8">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h2 class="text-2xl font-bold text-white">Informe Académico</h2>
            <p class="text-white/40 text-sm">{informeData.p2} - {informeData.p3} - Año {informeData.anio}</p>
          </div>
          <div class="text-right">
            <p class="text-white/60 text-sm">Estudiante</p>
            <p class="text-white font-bold">{selectedRecord?.nombres2}</p>
          </div>
        </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-white/10">
                <th class="text-left py-3 px-4 text-white/40 text-sm font-medium">#</th>
                <th class="text-left py-3 px-4 text-white/40 text-sm font-medium">Área</th>
                <th class="text-left py-3 px-4 text-white/40 text-sm font-medium">Descripción del Logro</th>
                <th class="text-left py-3 px-4 text-white/40 text-sm font-medium">Valoración</th>
              </tr>
            </thead>
            <tbody>
              {#each informeData.parsed_informe as item}
                <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                  <td class="py-3 px-4 text-white/60 text-sm">{item.numero}</td>
                  <td class="py-3 px-4 text-white font-medium text-sm">{item.area}</td>
                  <td class="py-3 px-4 text-white/80 text-sm">{item.descripcion}</td>
        ...
        {#if informeData.observaciones}
          <div class="mt-6 p-4 rounded-xl bg-white/5 border border-white/5">
            <p class="text-white/40 text-sm font-medium mb-2">Observaciones</p>
            <p class="text-white/80 text-sm">{informeData.observaciones}</p>
          </div>
        {/if}
      </div>
    {/if}
...
{#if showModal}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative w-full max-w-lg glass-card rounded-3xl p-6 border border-white/10 shadow-2xl">
      <div class="flex items-center justify-between mb-6">
        <h3 class="text-xl font-bold text-white">Buscar estudiante</h3>
      ...
      <div class="relative mb-4">
        <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
        </svg>
        <input
          type="text"
          ...
          class="w-full pl-12 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-white/30 focus:outline-none focus:border-purple-500/50 transition-colors"
        />
      </div>
      ...
      <div class="max-h-80 overflow-y-auto">
        ...
        {:else if searchResults.length === 0}
          <div class="text-center py-8">
            <p class="text-white/30 text-sm">{searchQuery ? 'No se encontraron resultados' : 'Escribe para buscar'}</p>
          </div>
        {:else}
          <div class="space-y-2">
            {#each searchResults as record}
              <button
                ...
                class="w-full p-4 rounded-xl bg-white/5 hover:bg-purple-500/10 border border-white/5 hover:border-purple-500/30 transition-all duration-200 text-left"
              >
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-white font-medium truncate">{record.nombres2}</p>
                    <p class="text-white/40 text-sm">IND: {record.ind}</p>
                  </div>
                </div>
              </button>
            {/each}
